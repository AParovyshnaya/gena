...
class Index():
    ...
    def deploy(self):
        ...
        Appendix(os.getcwd().replace("\\", "/")[:-3:] + 'files/css/bootstrap.min.css', self.output + "site/css/bootstrap.min.css").copy()
        Appendix(os.getcwd().replace("\\", "/")[:-3:] + 'files/css/main.css', self.output + "site/css/main.css").copy()
        return index
