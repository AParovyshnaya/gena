...
class Index():
    ...
    def deploy(self):
        ...
        Image(self.input + 'images/site/icon.png', self.output + 'site/images/site/icon.png').deploy()
        Appendix(os.getcwd().replace("\\", "/")[:os.getcwd().replace("\\", "/").rindex("/") + 1:] + 'src/files/css/bootstrap.min.css', self.output + "site/css/bootstrap.min.css").copy()
        Appendix(os.getcwd().replace("\\", "/")[:os.getcwd().replace("\\", "/").rindex("/") + 1:] + 'src/files/css/main.css', self.output + "site/css/main.css").copy()
